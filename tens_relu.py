...
# =============================================================================
def add_layer(inputs, input_size, output_size, trainable = True, w_lock = False, activation_function = None, relu_unit="0"):
    weightname = "weight-"+relu_unit
    biasname = "bias-"+relu_unit
    if not (w_lock):
        W = tf.Variable(np.random.uniform(-1, 1, size = (input_size, output_size)), trainable = trainable, name=weightname)
        b = tf.Variable(np.random.uniform(-1, 1, size =output_size), trainable = trainable,name=biasname)
        output = tf.matmul(inputs, W) + b
    else:
        W = tf.Variable(np.ones((input_size,output_size)), trainable = trainable, name=weightname)
...
def hey(datafile, test_error = False, goal='oil', grid_size = 8, plot = False,learning_rate=0.03, factor = 1.5, cross_validation = None,
        epochs = 500, beta = 0.2, val_interval = None, train_frac = 1.0, val_frac = 0.0, n_hidden = 30,
    ...
    if (prev == None):
        ...
        L1, W1, b1 = add_layer(x, 1, 20, activation_function = tf.nn.relu, relu_unit="1-1")
        L12, W12, b12 = add_layer(L1, 20, 1, w_lock=1, trainable=False, relu_unit="1-2")
        regularizers = tf.nn.l2_loss(W1)
        dropout1 = tf.nn.dropout(L12, keep_prob)
        loss = tf.reduce_mean(tf.square(y_ - dropout1) + beta * regularizers)
        train_step = tf.train.AdamOptimizer(learning_rate).minimize(loss)
        sess = tf.InteractiveSession()
        prev[0] = [sess, train_step, x, y_, keep_prob, dropout1, loss]
        ...
        L22, W22, b22 = add_layer(L2, n_hidden, 1, w_lock=1, trainable=False, relu_unit="2-2")
        regularizers2 = tf.nn.l2_loss(W2)
        dropout2 = tf.nn.dropout(L22, keep_prob2)
        loss2 = tf.reduce_mean(tf.square(y_2 - dropout2) + beta * regularizers2)
        train_step2 = tf.train.AdamOptimizer(learning_rate).minimize(loss2)
        prev[1] = [sess, train_step2, x2, y_2, keep_prob2, dropout2, loss2]
...
def run(datafile, prev, test_error=False, goal='oil', grid_size = 8, plot = False, factor = 1.5, cross_validation = None,
    ...
    if (cross_validation == None):
        ...
        for i in range(epochs + 1):
            batch_xs, batch_ys = tens.next_batch(train_set, len(train_set))
